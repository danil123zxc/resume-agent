...
from app.agents.resume_optimizer import extract_keywords, rewrite_resume_with_agent
...
async def optimize_resume(payload: OptimizeRequest) -> OptimizeResponse:
    try:
        extracted_keywords = await extract_keywords(payload.jobPostText)
        optimized_resume_text = await rewrite_resume_with_agent(
